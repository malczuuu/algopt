...
static inline int matr_index(int size, int i, int j)
{
    return i + size * j;
}
...
void matvec_avx(double *y_vect, const double *a_matr, const double *x_vect, int size)
{
    ...
    for (int i = 0; i < size; i += outer_unwinding)
    {
        ...
        {
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * inner_unwinding), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 1)), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 2)), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 3)), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 4)), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 5)), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 6)), _MM_HINT_NTA);
            _mm_prefetch((const char *) (ptr_a + outer_unwinding * (inner_unwinding + 7)), _MM_HINT_NTA);

            _mm_prefetch((const char *) (ptr_x + inner_unwinding), _MM_HINT_T0);
